 ...
 *      15、复合模式：MVC是复合模式，结合了观察者模式、策略模式和组合模式
 *          模型使用观察者模式，以便观察者更新，同时保持两者之间的解耦
 *          控制器是视图的策略，视图可以使用不同的控制器实现，得到不同的行为
 *          视图使用组合模式实现，用户界面通常组合了嵌套的组件，像面板、框架和按钮。
 *      16、责任链模式：避免请求发送者与接收者耦合在一起，让多个对象都有可能接收请求，将这些对象连接成一条链，并且沿着这条链传递请求，直到有对象处理它为止。
 *      17、所谓模式就是在某个情景之下针对某个问题的解决方案
 *
 *      一句话：
