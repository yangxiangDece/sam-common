 ...
 * Redisson分布式锁
 */
public class RedissonDistributedLockTool {

    /**
     * 可重入锁 同步获取
     * Redisson的分布式可重入锁RLock Java对象实现了java.util.concurrent.locks.Lock接口，同时还支持自动过期解锁。
     */
    public static void testReentrantLock(RedissonClient redisson){
    ...
    /**
     * 公平锁 同步获取
     * Redisson分布式可重入公平锁也是实现了java.util.concurrent.locks.Lock接口的一种RLock对象。在提供了自动过期解锁功能的同时，
...
//            boolean res = lock.tryLock(100, 10, TimeUnit.SECONDS);
    ...
    public void testReadWriteLock(RedissonClient redisson){
        RReadWriteLock readWriteLock = redisson.getReadWriteLock("anyRWLock");
        // 最常见的使用方法
        ...
        // 尝试加锁，最多等待100秒，上锁以后10秒自动解锁
        try {
            boolean res = readWriteLock.readLock().tryLock(100, 10, TimeUnit.SECONDS);
            // 或
//            boolean res = readWriteLock.writeLock().tryLock(100, 10, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
     ...
     *
     */
    public void testSemaphore(RedissonClient redisson){
     ...
     * Redisson的可过期性信号量（PermitExpirableSemaphore）实在RSemaphore对象的基础上，为每个信号增加了一个过期时间。
     * 每个信号可以通过独立的ID来辨识，释放时只能通过提交这个ID才能释放。
     */
    public void testExpirableSemaphore(RedissonClient redisson){
        RPermitExpirableSemaphore expirableSemaphore = redisson.getPermitExpirableSemaphore("mySemaphore");
        try {
            String permitId = expirableSemaphore.acquire();
            // 获取一个信号，有效期只有2秒钟。
            String permitIds = expirableSemaphore.acquire(2, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
     ...
     *
     */
    public void testCountDownLatch(RedissonClient redisson){
        ...
        try {
            ...
            // 在其他线程或其他JVM里
            RCountDownLatch latchJ = redisson.getCountDownLatch("anyCountDownLatch");
            latchJ.countDown();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
